// 引入 score.js
import {  displayGameResults } from './scoreList.js';
...
function updateScore(snake){
  if(snake.id ==1){
    snake.score++;
    document.getElementById('player1-score').textContent = `Player 1: ${snake.score}`;
  }else if(snake.id ==2){
    snake.score++;
    document.getElementById('player2-score').textContent = `Player 2: ${snake.score}`;
  }
}
initializeFood();
initializeSnakes();
